...
function getImportantExif(array $exif): array
{
    $priorityKeys = [
        ...
        'DateTime', 'DateTimeOriginal', 'ExposureTime', 'FNumber',
        'ISOSpeedRatings', 'FocalLength', 'Flash', 'WhiteBalance',
        'MeteringMode', 'ExposureProgram', 'LensModel', 'UserComment',
        'GPSLatitude', 'GPSLongitude', 'GPSAltitude', 'XPTitle', 'XPSubject', 'XPComment',
    ];
...
<head>
    ...
    <style>
        body { margin: 0; font-family: ui-sans-serif, system-ui, -apple-system, BlinkMacSystemFont, 'Segoe UI', sans-serif; background: #f4f7fb; color: #111827; }
        .container { max-width: 1040px; margin: 0 auto; padding: 24px; }
        .top-bar { display: flex; flex-wrap: wrap; justify-content: space-between; gap: 12px; align-items: center; margin-bottom: 24px; }
        .top-bar a { color: #2563eb; text-decoration: none; font-weight: 600; }
        .panel { background: #ffffff; border: 1px solid #d6dde8; border-radius: 20px; padding: 24px; box-shadow: 0 16px 40px rgba(15,23,42,.08); }
        .detail-grid { display: grid; grid-template-columns: 1.2fr 0.8fr; gap: 24px; }
        .detail-image { border-radius: 20px; overflow: hidden; }
        .detail-image img { width: 100%; height: auto; display: block; }
        .info-box { display: grid; gap: 18px; }
        .info-box h1 { margin: 0; font-size: 1.8rem; }
        .info-box p { margin: 0; color: #475569; line-height: 1.75; }
        .info-table, .exif-table { width: 100%; border-collapse: collapse; }
        .info-table th, .info-table td, .exif-table th, .exif-table td { padding: 12px 14px; border-bottom: 1px solid #e5e7eb; }
        .info-table th, .exif-table th { text-align: left; background: #f8fafc; width: 35%; }
        .section-title { margin: 0 0 14px; font-size: 1.15rem; }
        .badge-row { display: flex; flex-wrap: wrap; gap: 10px; margin-top: 10px; }
        .badge { display: inline-flex; align-items: center; padding: 8px 12px; border-radius: 999px; background: #eff6ff; color: #1d4ed8; font-size: 0.95rem; }
        .footer { margin-top: 28px; font-size: 0.95rem; color: #475569; }
...
<div class="container">
    ...
    <div class="panel">
        ...
        <div class="info-box">
                <div>
                    <h1><?= h($title) ?></h1>
                    <p><?= h($description ?: '无描述') ?></p>
                    <p>点击图片查看原图或下载内容。</p>
                </div>
                <div>
                    <h2 class="section-title">基本信息</h2>
                    <table class="info-table">
                        <tr><th>文件名</th><td><?= h($filename) ?></td></tr>
                        <tr><th>大小</th><td><?= h(formatBytes($size)) ?></td></tr>
                        <tr><th>类型</th><td><?= h($mime) ?></td></tr>
                        <?php if ($resolution): ?><tr><th>分辨率</th><td><?= h($resolution) ?></td></tr><?php endif; ?>
                    </table>
                </div>
            </div>
        </div>
    ...
    <div class="footer">
        <p>如果你是管理员，可前往 <a href="upload.php">后台上传</a> 新图片。</p>
    </div>
</div>
</body>
</html>
